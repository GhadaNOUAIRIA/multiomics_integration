...
Progress: miRNA initial attempt
...
metadata <- metadata %>% 
  select(patient_id, cca_binary, ibd_binary, fibrosis_binary, alp_binary, bilirubin_binary, sex_binary, age, bmi, crp, IgG, IgA)
```
...
This initial plot groups the samples with hierarchical clustering. The abline is set manually after looking at the plot.
...
This function divides the data into the clusters that can be seen in the plot above. Then only the main cluster is kept and the data is referred to as fmiRNA.
...
We create a first network which tells us there are 13 variables with missing values or low variance. We then create the second network with those 13 variables filtered out. It divides our variables into 20 modules

```{r}
# First model

net0 = blockwiseModules(fmiRNA, power = 4,
...
fmiRNA <- fmiRNA[, net0[["goodGenes"]]]

net = blockwiseModules(fmiRNA, power = 4,
                       TOMType = "signed", minModuleSize = 30,
                       reassignThreshold = 0, mergeCutHeight = 0.25,
                       numericLabels = TRUE, pamRespectsDendro = FALSE,
                       saveTOMs = TRUE,
...
dissTOM = 1-TOMsimilarityFromExpr(fmiRNA, power = 6);
