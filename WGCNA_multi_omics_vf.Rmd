...
multi_module_disease_analysis <- function(modules, disease, show_venn = TRUE) {
  ...
  # --- 1. Disease-associated compounds --------------------------------------
  disease_file   <- paste0("../results/mixOmics_", disease, "_binary.csv")
  compounds_list <- readr::read_csv(disease_file, show_col_types = FALSE)$compounds

  # --- 2. Variables from all selected modules --------------------------------
  venn_lists <- list(Compounds = compounds_list)
  for (c in modules) {
    idx <- moduleMembershipDF$ModuleColor == c
    variables_list <- moduleMembershipDF$Variable[idx]
    venn_lists[[c]] <- variables_list
  }

  # Merge all module variables into a single union
  variables_list_union <- unique(unlist(venn_lists[names(venn_lists) != "Compounds"]))

  # --- 3. Single Venn diagram ------------------------------------------------
  if (show_venn) {
    ven <- ggVennDiagram::ggVennDiagram(venn_lists, label = "count") +
           ggplot2::theme_void()
    print(ven)
  }

  # --- 4. Filtered multi-omics dataset --------------------------------------
  merged_list         <- base::union(compounds_list, variables_list_union)
  filtered_multiomics <- multi_omics[
    , base::intersect(colnames(multi_omics), merged_list), drop = FALSE
  ]
...
# -----------------------------------------------------------------------------
# Helper: retrieve KEGG compound name for a single identifier ------------------
get_kegg_name <- function(id) {
  url <- paste0("https://rest.kegg.jp/list/cpd:", id)
  res <- httr::GET(url)
...
# Helper: retrieve UniProt recommended protein name ----------------------------
...
# -----------------------------------------------------------------------------
# Main KEGG multi-omics pipeline ----------------------------------------------
multi_module_kegg_pipeline <- function(modules,
  ...
  suppressPackageStartupMessages({
    ...
    library(clusterProfiler); library(enrichplot);  library(ggplot2)
    library(metap);  library(org.Hs.eg.db); library(ggVennDiagram)
    library(tibble); library(httr); library(jsonlite)
  })

  ## 1) Build the combined multi-omics dataset --------------------------------
  if (verbose) message("➜ Step 1: building the multi-omics dataset …")
  multi_omics_mod <- multi_module_disease_analysis(
    modules, disease, show_venn = verbose
  )

  ## 2) Split variables into genes / proteins / metabolites -------------------
  if (verbose) message("➜ Step 2: splitting genes / proteins / metabolites …")
  gene_cols <- grep("^hsa[-_]", colnames(multi_omics_mod),
                    value = TRUE, ignore.case = TRUE)
  prot_cols <- grep("^OID", colnames(multi_omics_mod), value = TRUE)
  met_cols  <- grep("^(X)?[0-9]+$", colnames(multi_omics_mod), value = TRUE)

  ## 3) Run KEGG enrichment analyses ------------------------------------------
  if (verbose) message("➜ Step 3: running KEGG enrichments …")

  # --- 3a) Metabolites -------------------------------------------------------
  kegg_unique <- medata_metabolites %>%
    dplyr::filter(as.character(`COMP ID`) %in% met_cols,
                  !is.na(KEGG), !KEGG %in% c("NA", "undefined")) %>%
    dplyr::pull(KEGG) |>
    strsplit(",") |> unlist() |> trimws() |> unique()

  ora_kegg_cpd <- clusterProfiler::enrichKEGG(
    gene = kegg_unique, organism = "cpd",
  ...
  prot_interest <- oid_map %>%
    dplyr::filter(oid %in% prot_cols) %>%
    dplyr::pull(uniprot) %>%
    unique()

  ora_kegg_prot <- clusterProfiler::enrichKEGG(
    gene = prot_interest, organism = "hsa",
  ...
  gene_interest <- mirna2gene %>%
    dplyr::filter(mature_mirna_id %in% gene_cols) %>%
    dplyr::pull(target_symbol) %>%
    stats::na.omit() %>% unique()

  eg <- clusterProfiler::bitr(
    gene_interest, fromType = "SYMBOL",
    toType = "ENTREZID", OrgDb = org.Hs.eg.db
  )

  ora_kegg_gene <- clusterProfiler::enrichKEGG(
    gene = eg$ENTREZID, organism = "hsa",
    keyType = "kegg", pvalueCutoff = 1,
    pAdjustMethod = "BH", maxGSSize = 500
  )

  ## 4) Meta-analysis of pathway p-values (Stouffer method) -------------------
  ...
  merged_kegg <- list(gene_df, prot_df, met_df) %>%
    purrr::reduce(dplyr::full_join, by = "path_num") %>%
    dplyr::mutate(description = dplyr::coalesce(description.x,
                                                 description.y,
                                                 description)) %>%
    dplyr::select(path_num, description, dplyr::ends_with("_padj")) %>%
    dplyr::mutate(
      p_comb_stouffer = apply(dplyr::across(dplyr::ends_with("_padj")), 1, \(row) {
        vals <- row[!is.na(row)]
        vals[vals == 0] <- .Machine$double.xmin
        if (length(vals) >= 2) return(metap::sumz(vals)$p)
        if (length(vals) == 1) return(vals)
        NA_real_
      }),
      p_comb_stouffer_adj = stats::p.adjust(p_comb_stouffer, method = "BH")
    )

  sig <- merged_kegg %>% dplyr::filter(p_comb_stouffer_adj <= p_cutoff)

  ## 5) Treeplot of significant KEGG pathways ---------------------------------
  ...
  gset <- function(er) er@result %>%
    ...
    dplyr::transmute(
      path_num = normalize_pathway(ID),
      gs = purrr::map(geneID, split_slash)
    )

  genesets_all <- dplyr::bind_rows(
    gset(ora_kegg_gene),
    gset(ora_kegg_prot),
    gset(ora_kegg_cpd)
  ) %>%
    dplyr::group_by(path_num) %>%
    dplyr::summarise(
      geneSets = list(unique(unlist(gs))),
      Count    = lengths(geneSets),
      .groups  = "drop"
    )

  sig2 <- sig %>% dplyr::left_join(genesets_all, by = "path_num")
  readr::write_csv(sig2, paste0(disease, "_enrich_table.csv"))

  # Rank pathways by number of omics layers
  sig2_ranked <- sig2 %>%
    dplyr::mutate(
      nOmics = (!is.na(gene_padj)) +
               (!is.na(prot_padj)) +
               (!is.na(met_padj))
    ) %>%
    dplyr::arrange(dplyr::desc(nOmics), p_comb_stouffer_adj)
  ...
  res_df <- sig2 %>%
    dplyr::transmute(
      ...
      p.adjust    = p_comb_stouffer_adj,
      qvalue      = p_comb_stouffer_adj,
      geneID      = purrr::map_chr(geneSets, paste, collapse = "/"),
      Count,
  ...
  er <- methods::new(
    "enrichResult",
    result        = res_df,
    geneSets      = stats::setNames(sig2$geneSets, sig2$path_num),
    pvalueCutoff  = 1,
    pAdjustMethod = "none",
    qvalueCutoff  = 1,
    organism      = "UNKNOWN",
  ...
  # Compute Jaccard similarity between pathways
  gs_list <- stats::setNames(
    purrr::map(er@result$geneID,
    ...
    for (j in i:n) {
      ...
      jac[i, j] <- jac[j, i] <-
        ifelse(length(unique(c(gi, gj))) == 0, 0,
               length(intersect(gi, gj)) /
               length(unique(c(gi, gj))))
    }
  }

  lab_vec <- with(er@result, stats::setNames(Description, ID))
  rownames(jac)  <- colnames(jac)  <- lab_vec[rownames(jac)]
  names(gs_list) <- lab_vec[names(gs_list)]
  er@result$ID   <- lab_vec[er@result$ID]
  ...
  cluster_cols <- c("#2ca02c", "#33caff", "#9467bd", "#d62727", "#ffdf33")
  ...
  ggplot2::ggsave(out_pdf, p, width = 16.5, height = 11.7, units = "in")

  ## 6) Majority-omics heatmap by cluster -------------------------------------
  if (verbose) message("➜ Step 6: building the majority-omics heat-map …")

  min_prop <- 0.74

  term_to_ids <- stats::setNames(
    purrr::map(er@result$geneID,
               \(s) unique(strsplit(s, "/", fixed = TRUE)[[1]])),
    er@result$ID
  )
  ...
  # Convert numeric gene IDs to miRNAs; keep other identifiers unchanged
  geneid_to_symbol <- stats::setNames(eg$SYMBOL, as.character(eg$ENTREZID))
  symbol_to_mirnas <- base::split(
    mirna_symbol$mature_mirna_id,
    mirna_symbol$target_symbol
  )
  ...
  ids2miRNAs_keep_others <- function(ids) {
    ...
    gene_ids  <- ids[id_is_numeric]
    other_ids <- ids[!id_is_numeric]
    syms <- geneid_to_symbol[gene_ids] |> stats::na.omit()
    miRs <- unique(unlist(symbol_to_mirnas[syms]))
    unique(c(other_ids, miRs))
  }
  ...
  # Rename KEGG compounds and UniProt proteins for readability
  df$id <- as.character(df$id)
  id_is_kegg    <- grepl("^C\\d{5}$", df$id)
  id_is_uniprot <- grepl("^[OPQ][0-9][A-Z0-9]{3}[0-9]$", df$id)
  ...
  df <- df %>%
    dplyr::mutate(id = if_else(id %in% names(name_map),
                              name_map[id], id)) %>%
    dplyr::mutate(
      # Omics identifiers on the x-axis (sorted)
      id      = factor(id, levels = sort(unique(id))),
      # Clusters on the y-axis
      cluster = factor(cluster,
                       levels = names(representative_mixed_by_cluster))
    )

  cluster_cols_named <- stats::setNames(
    cluster_cols,
    names(representative_mixed_by_cluster)
  )

  heat_title <- paste0(
    "Majority omics by cluster – ",
    disease, " – ",
    paste(modules, collapse = ", ")
  )

  # Clusters on rows, omics on columns
  p_heat <- ggplot2::ggplot(df, ggplot2::aes(x = id, y = cluster)) +
    ggplot2::geom_tile(
    ...
    ggplot2::scale_fill_manual(values = cluster_cols_named, guide = "none") +
    ggplot2::labs(title = heat_title, x = NULL, y = "Cluster") +
  ...
  # Horizontal layout suitable for manuscript figures
  ggplot2::ggsave(heatmap_pdf, p_heat, width = 14, height = 5, units = "in")
  if (verbose) message("✓ Heat-map saved as \"", heatmap_pdf, "\".")

  ## 7) Return useful objects -------------------------------------------------
  invisible(list(
