...
##7. Enrichissement par module + diablo pour KEGG
...
### Treeplot sans priorité aux 3 omics
...
### Treeplot avec priorité aux 3 omics
...
cluster_cols <- c(
  "#2ca02c",  # Rouge  (cluster_1)
  "#33caff",  # Vert   (cluster_2)
  "#9467bd",  # Bleu   (cluster_3)
  "#d62727",  # Jaune  (cluster_4)
  "#ffdf33"   # Violet (cluster_5)
)

# 2) appel direct : on fournit la palette via cluster.params$color
p <- treeplot(
  er,
  showCategory   = show_vec,
  cluster.params = list(
    method = "average",   # ou ward.D, etc.
    n      = 5,           # nb de groupes
    color  = cluster_cols # <- palette perso
  ),
  label_format   = 60,     # comme avant
  color          = "p.adjust" # laisse la scale continue pour les points
)
...
### Extraction des omics unique par term et cluster 
...
min_prop <- 0.75          # ≥ 75 % des termes du cluster

## Objets déjà en mémoire :
##   • er              : enrichResult
##   • p               : treeplot – on y prend p[["data"]]
##   • eg              : data.frame  SYMBOL, ENTREZID
##   • mirna_symbol    : data.frame  mature_mirna_id, target_symbol

###############################################################################
## 1)  Table globale TERME → IDENTIFIANTS (gènes, composés, protéines…)
###############################################################################
...
for (cl in cluster_ids) {
  ...
  ## a) Termes (labels) du cluster
  terms <- p[["data"]] |>
    filter(isTip, group == cl, !is.na(label)) |>
    pull(label) |>
    unique()
  if (!length(terms)) next   # cluster vide → on saute

  ## b) Sous-ensemble TERME → ids pour ce cluster
  ids_by_term <- term_to_ids[terms]

  ## c) Identifiants représentatifs (≥ min_prop des termes)
  freq <- table(unlist(ids_by_term))
  representative_ids_by_cluster[[cl]] <-
    sort(names(freq)[freq / length(ids_by_term) >= min_prop])

  ## d) Identifiants uniques à chaque terme  (unicité globale)
...
###############################################################################
## 5)  Conversion des identifiants GÈNES (numériques) → miRNA
##     – on garde intact tout identifiant contenant des lettres
###############################################################################
...
## Fonction : ids mixtes → miRNA (pour les gènes) + ids non-gènes inchangés
ids2miRNAs_keep_others <- function(ids) {
  ids           <- as.character(ids)               # sécurité
  id_is_numeric <- grepl("^\\d+$", ids)
  gene_ids      <- ids[id_is_numeric]
  other_ids     <- ids[!id_is_numeric]

  ## GENEID → SYMBOL
  syms <- geneid_to_symbol[gene_ids]
...
## a) Représentatifs : miRNA + composés/protéines inchangés
representative_mixed_by_cluster <-
  lapply(representative_ids_by_cluster, ids2miRNAs_keep_others)

## b) Uniques par terme  (‼️ double lapply)
unique_mixed_by_term_by_cluster <-
  lapply(unique_ids_by_term_by_cluster, function(clist)
    lapply(clist, ids2miRNAs_keep_others))


###############################################################################
## 6)  Objets finaux
##     • representative_mixed_by_cluster        # miRNA + ids non gènes
##     • unique_mixed_by_term_by_cluster        # idem, par terme & cluster
###############################################################################
########################################################################
...
```{r}
## ── 0. Pré‑requis ------------------------------------------------------------
library(tidyverse)   # ggplot2 + purrr + tibble…

## ── 1. Récupérer l’objet liste ----------------------------------------------
# Supposons qu’il est déjà en mémoire:
#   str(representative_mixed_by_cluster)
#   List of 5
clusters <- representative_mixed_by_cluster          # plus lisible

## ── 2. Transformer au format «long» ----------------------------------------
df <- imap_dfr(clusters, ~tibble(id = .x, cluster = .y))

# axe Y dans un ordre stable (alphabétique, puis renversé pour affichage top‑down)
all_ids <- sort(unique(df$id))
df$id   <- factor(df$id, levels = rev(all_ids))

# assure l’ordre des clusters (colonne X)
df$cluster <- factor(df$cluster, levels = names(clusters))

## ── 3. Palette manuelle ------------------------------------------------------
cluster_cols <- c(
  cluster_1 =   "#2ca02c",  # Rouge  (cluster_1)
  cluster_2 =  "#33caff",  # Vert   (cluster_2)
  cluster_3 = "#9467bd",  # Bleu   (cluster_3)
  cluster_4 = "#d62727",  # Jaune  (cluster_4)
  cluster_5 = "#ffdf33"
)[names(clusters)]          # garde l’ordre exact
...
p <- ggplot(df, aes(x = cluster, y = id)) +
  ...
  labs(
    title = "Omics présents dans 75% des clusters",
    x = "Cluster",
    y = NULL
  ) +
...
## ── 5. Sauvegarde PDF grand format ------------------------------------------
pdf("cluster_heatmap.pdf", width = 10, height = 14)  # format portrait XXL
print(p)
dev.off()

# ➜ Le fichier 'cluster_heatmap.pdf' est prêt dans le répertoire de travail.
#   Utilise par ex. getwd() pour le localiser si besoin.
```
...
# ---- fonction pour UN identifiant ----
get_kegg_name <- function(id) {
  url <- paste0("https://rest.kegg.jp/list/cpd:", id)   # endpoint « list »
  res <- GET(url)
  stop_for_status(res)
...
multi_module_kegg_pipeline <- function(modules,
  ...
  suppressPackageStartupMessages({
    ...
    library(clusterProfiler); library(enrichplot);    library(ggplot2)
    library(metap);  library(org.Hs.eg.db);           library(ggVennDiagram)
    library(tibble)         # pour tibble::tibble
  })
  ...
  ## 5d) Similarité Jaccard + renommage ---------------------------------------
  gs_list <- setNames(
  ...
  ## 6)  HEAT‑MAP « MAJORITY OMICS » ------------------------------------------
  if (verbose) message("➜ Étape 6 : heat‑map majority omics…")

  min_prop <- 0.75                        # ≥ 75 % des termes du cluster

  # 6a) Terme → identifiants bruts
  term_to_ids <- setNames(
    map(er@result$geneID, \(s) unique(strsplit(s, "/", fixed = TRUE)[[1]])),
    er@result$ID
  )

  # 6b) Liste des clusters présents dans le treeplot
  cluster_ids <- p[["data"]] |>
    filter(isTip, !is.na(group), grepl("^cluster_", group)) |>
    pull(group) |> unique()
  ...
  # 6c) Conversion gène numérique → miRNA (autres id inchangés)
  geneid_to_symbol <- setNames(eg$SYMBOL, as.character(eg$ENTREZID))
  symbol_to_mirnas <- split(mirna_symbol$mature_mirna_id,
                            mirna_symbol$target_symbol)
  ...
  print(representative_mixed_by_cluster)
  
  
  
  # 6d) Données longues pour ggplot
  # 6d) Données longues pour ggplot
  ...
  # ---------------------------------------------------------------------------
  # 1. id en caractère pour toute la suite
  df$id <- as.character(df$id)
  
  # 2. Détection des ids à renommer -------------------------------------------
  id_is_kegg    <- grepl("^C\\d{5}$",        df$id)   # composés KEGG
  id_is_uniprot <- grepl("^[OPQ][0-9][A-Z0-9]{3}[0-9]$", df$id)  # UniProt 6 car.
  
  kegg_ids <- unique(df$id[id_is_kegg])
  prot_ids <- unique(df$id[id_is_uniprot])
  
  # 3. Récupération des noms ---------------------------------------------------
  ...
  # 4. Substitution ------------------------------------------------------------
  name_map <- c(kegg_names, prot_names)      # vecteur « ID = nom »
  
  df <- df %>%
    mutate(id = if_else(id %in% names(name_map),
                        name_map[id],
                        id))
  
  # 5. (optionnel) remettre en facteur pour l’ordre de la heat‑map -------------
  df <- df %>%
    mutate(
  ...
  if (verbose) message("✓ Heat‑map sauvegardée dans « ", heatmap_pdf, " ».")
...
# ---- vecteur d’identifiants ----
ids <- c("O00585", "O14625", "Q9Y2T1")

# ---- appel vectorisé avec vapply pour garder les noms dans le même ordre ----
prot_names <- vapply(ids, get_uniprot_name, character(1))
...
# ---- vecteur d’IDs ----
ids <- c("C00186", "C00031", "C00311")

# ---- appel vectorisé ----
kegg_names <- vapply(ids, get_kegg_name, character(1))

# kegg_names est le vecteur que vous avez déjà obtenu,
# ex. c("C00186" = "(S)-Lactate; L-Lactate; L-Lactic acid", ...)

kegg_names_first <- sub(";.*$", "", kegg_names) |>     # enlève tout après le 1ᵉʳ « ; »
                     trimws()                          # supprime espaces de début/fin
kegg_names_first
...
df <- tribble(
  ~id,             ~cluster,
  # miRNA (commencent par « hsa‑ »)
  "hsa-miR-15a-5p","cluster_1",
  "hsa-miR-29c-3p","cluster_1",
  "hsa-let-7a-5p", "cluster_1",
  # composés KEGG (Cxxxxx)
  "C00186",        "cluster_2",
  "C00031",        "cluster_2",
  # protéines UniProt (O/P/Q + 5 caractères alphanumériques)
  "Q07325",        "cluster_2",
  "P80098",        "cluster_2",
)

id_is_kegg   <- grepl("^C\\d{5}$",       df$id)                  # C00186, C00311…
id_is_uniprot<- grepl("^[OPQ][0-9][A-Z0-9]{3}[0-9]$", df$id)     # O00585, P01588…
...
kegg_names   <- sub(";.*$", "", kegg_raw) |>         # garde la partie avant le 1ᵉʳ « ; »
                 trimws()                            # supprime espaces parasites

#‑‑ UniProt protéines ---------------------------------------------------------
prot_names   <- vapply(prot_ids, get_uniprot_name, character(1), USE.NAMES = TRUE)

###############################################################################
# 3. Dictionnaire ID → nom & remplacement dans df ----------------------------
###############################################################################
name_map     <- c(kegg_names, prot_names)            # vecteur nommé

df <- df |>
  dplyr::mutate(id = dplyr::if_else(id %in% names(name_map),
                                    name_map[id],     # remplace
                                    id))              # sinon garde l’ID d’origine
